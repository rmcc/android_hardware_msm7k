...
OSCL_EXPORT_REF bool AndroidSurfaceOutputFB::initCheck()
{
    ...
    if (((iVideoParameterFlags & VIDEO_SUBFORMAT_VALID) == 0) || !checkVideoParameterFlags())
        ...
        LOGV("passed initialization params");
...
OSCL_EXPORT_REF PVMFStatus AndroidSurfaceOutputFB::writeFrameBuf(uint8* aData, uint32 aDataLen, const PvmiMediaXferHeader& data_header_info)
{
...
OSCL_EXPORT_REF void AndroidSurfaceOutputFB::closeFrameBuf()
{
    LOGV("CloseFrameBuf");
...
extern "C" OSCL_EXPORT_REF AndroidSurfaceOutput* createVideoMio()
{
    LOGV("createVideoMio was invoked");
